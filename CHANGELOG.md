...
- **Reconcile device `status` drift** ([ADR-0010](docs/architecture/adr/0010-management-plane-contract.md)
  ...
  field — no FK resolution). The UniFi pack maps its raw device `state` to a NetBox status with a
  deliberately conservative table (`ONLINE`→`active`, `OFFLINE`→`offline`,
  `PENDING_ADOPTION`/`ADOPTING`→`staged`); every other / unknown / transient state
  (`UPDATING`/`PROVISIONING`/…) is left unmapped (`None`), so reconcile skips it and a sparse or
  transient discovery run never blanks out or churns the status NetBox already holds. The mapping
  is the single source of truth in `discovery/vendors/unifi/models.py`. `platform` stays
  deferred. (#119)
- **Reconcile device `serial` drift** ([ADR-0010](docs/architecture/adr/0010-management-plane-contract.md)
  ...
  untouched. `platform` (no clean NetBox target) remains deferred. (#119)
- **Drift comparison now covers `device_type` + `manufacturer`**: reconcile detects (and, on
  ...
  `device_type` foreign key on apply. `platform` is deferred. (#74)
...
- **Verify NetBox webhook signatures**: `POST /webhooks/netbox` now verifies NetBox's
  ...
  verification (back-compat). The bundled `deploy/` compose stack and the Ansible
...
[Unreleased]: https://github.com/freed-dev-llc/argus/compare/v0.2.8...HEAD
[0.2.8]: https://github.com/freed-dev-llc/argus/compare/v0.2.7...v0.2.8
